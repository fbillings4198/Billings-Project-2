...
function Hazard(pos) {
  ...
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.haz = Math.random() * Math.PI * 2;
}
...
var hazSpeed = 1, hazDist = 33;

Hazard.prototype.act = function(step) {
  this.haz += step * hazSpeed;
  var hazPos = Math.sin(this.haz) * hazDist;
  this.pos = this.basePos.plus(new Vector(hazPos, 0));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if (type == "lava" || type == "hazard" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
